...
const initialPremiumNudges: PremiumNudgeProps[] = [
  {
    ...
    description:
      "Learn techniques to mediate and resolve team conflicts effectively",
    cost: 50,
  ...
  },
  {
    ...
    description:
      "Framework for making high-impact decisions with limited information",
    cost: 75,
    category: "Leadership",
    unlocked: false,
  },
  {
    ...
    description:
      "Develop advanced EQ skills to better understand and motivate your team",
    cost: 100,
  ...
  },
  {
    ...
    description:
      "Transform your one-on-one meetings into powerful coaching sessions",
    cost: 40,
  ...
  },
  {
    ...
    description:
      "Leading effectively during times of uncertainty and rapid change",
    cost: 90,
    category: "Leadership",
    unlocked: false,
  },
...
export function GamificationProvider({
  ...
  const [premiumNudges, setPremiumNudges] =
    useState<PremiumNudgeProps[]>(initialPremiumNudges);
  ...
  const calculateLevelInfo = () => {
    const currentLevelData =
      levelData.find((l) => xp >= l.minXp && xp < l.maxXp) ||
      levelData[levelData.length - 1];

    const progress = Math.round(
      ((xp - currentLevelData.minXp) /
  ...
  const unlockPremiumNudge = (nudgeId: string): boolean => {
    ...
    if (nudge && !nudge.unlocked) {
      if (spendCoins(nudge.cost)) {
        setPremiumNudges((prev) =>
          prev.map((n) => (n.id === nudgeId ? { ...n, unlocked: true } : n))
        );
        return true;
      }
    }
  ...
  const incrementStreak = () => {
    ...
    const lastDate = streak.lastCompleted
    ...
    if (
      !lastDate ||
      currentDate.getTime() - lastDate.getTime() > 86400000 * 2
    ) {
