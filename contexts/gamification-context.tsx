...
interface GamificationContextType {
  ...
  // New properties for the new features
  freezePasses: number
  addFreezePass: (amount: number) => void
  useFreezePass: () => boolean
  boostedNudges: string[]
  boostNudge: (nudgeId: string) => boolean
  aiCoachingHistory: {
    id: string
    question: string
    answer: string
  ...
  addCoachingInteraction: (question: string, answer: string, cost: number) => void
  feedbackSubmitted: boolean
  setFeedbackSubmitted: (submitted: boolean) => void
  // Community sharing properties
  completedNudge: (nudgeTitle: string) => void
}
...
export function GamificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const [boostedNudges, setBoostedNudges] = useState<string[]>([])
  const [aiCoachingHistory, setAiCoachingHistory] = useState<
    {
      id: string
      question: string
      answer: string
  ...
  // Get community context for sharing
  const communityContextRef = useRef<any>(null)
  useEffect(() => {
    try {
      communityContextRef.current = useContext(require("@/contexts/community-context").CommunityContext)
    } catch (error) {
      // Community context might not be available yet
    }
  }, [])
  ...
  useEffect(() => {
    ...
    if (newLevelInfo.currentLevel !== level) {
      ...
      if (newLevelInfo.currentLevel > level) {
        ...
        // Trigger sharing prompt for level up
        if (communityContextRef.current) {
          communityContextRef.current.setShowSharingPrompt(true)
          communityContextRef.current.setSharingData({
            type: "levels",
            title: `Level Up to ${newLevelInfo.title}!`,
            content: `I just reached ${newLevelInfo.title} level! The journey continues with new challenges and opportunities. #levelup #leadership`,
          })
        }
      }
    }
  ...
  const earnBadge = (badgeId: string) => {
    ...
    if (badge && !badge.earned) {
      ...
      // Trigger sharing prompt for badge
      if (communityContextRef.current) {
        communityContextRef.current.setShowSharingPrompt(true)
        communityContextRef.current.setSharingData({
          type: "badges",
          title: `New Badge: ${badge.name}!`,
          content: `I just earned the ${badge.name} badge! ${badge.description} #achievement #leadership`,
        })
      }
    }
  }
  ...
  const incrementStreak = () => {
    ...
    if (!lastDate || currentDate.getTime() - lastDate.getTime() > 86400000 * 2) {
      ...
      // Trigger sharing prompt for streak milestones
      if (
        communityContextRef.current &&
        (newCurrent === 3 || newCurrent === 7 || newCurrent === 14 || newCurrent === 30)
      ) {
        communityContextRef.current.setShowSharingPrompt(true)
        communityContextRef.current.setSharingData({
          type: "streaks",
          title: `${newCurrent}-Day Streak!`,
          content: `I'm on a ${newCurrent}-day streak! Consistency is key to building better leadership habits. #consistency #leadership #streak`,
        })
      }
    }
  }
  ...
  const addFreezePass = (amount: number) => {
    setFreezePasses((prev) => prev + amount)
    toast({
      title: "Streak Freeze Pass Acquired!",
      description: `You now have ${freezePasses + amount} freeze passes.`,
    })
  }

  const useFreezePass = (): boolean => {
    if (freezePasses > 0) {
      setFreezePasses((prev) => prev - 1)
      // Logic to protect streak
      setStreak((prev) => ({
        ...prev,
        lastCompleted: new Date().toISOString().split("T")[0],
        canRevive: false,
  ...
  const boostNudge = (nudgeId: string): boolean => {
    if (!boostedNudges.includes(nudgeId)) {
      setBoostedNudges((prev) => [...prev, nudgeId])
      return true
    }
    return false
  }

  const addCoachingInteraction = (question: string, answer: string, cost: number) => {
    setAiCoachingHistory((prev) => [
      ...prev,
      {
  ...
  // Method to handle completed nudge and trigger sharing prompt
  const completedNudge = (nudgeTitle: string) => {
    if (communityContextRef.current) {
      communityContextRef.current.setShowSharingPrompt(true)
      communityContextRef.current.setSharingData({
        type: "nudges",
        title: `Completed: ${nudgeTitle}`,
        content: `I just completed the "${nudgeTitle}" nudge! It was a valuable experience that helped me improve my leadership skills. #leadership #growth`,
      })
    }
  }
